...
public class Drag : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if (m_TargetJoint)
		{
            ...
            {
                var Card = m_TargetJoint.gameObject;
                StartCoroutine(FreezeCardDelay(Card));
            }
		}

	}

    IEnumerator FreezeCardDelay(GameObject Card)
    {
        var Card_sr = Card.GetComponent<SpriteRenderer>();
        var Card_rb = Card.GetComponent<Rigidbody2D>();

        Card_sr.color = Color.red;
        Card_rb.constraints = RigidbodyConstraints2D.FreezeRotation;
        yield return new WaitForSeconds(5f);
        Card_sr.color = Color.white;
        Card_rb.constraints = RigidbodyConstraints2D.None;
    }
}